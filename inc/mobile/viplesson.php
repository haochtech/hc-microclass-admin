...
$list = pdo_fetchall("SELECT id,bookname,images,price,section_status,score,buynum,virtual_buynum,vip_number,teacher_number,visit_number,section_status FROM " .tablename($this->table_lesson_parent). " WHERE {$condition} ORDER BY displayorder DESC  LIMIT " . ($pindex-1) * $psize . ',' . $psize, $params);
