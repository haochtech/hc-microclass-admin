...
if($comsetting['sale_rank']==2 && empty($memberVip)){
	message("您不是VIP会员，无法访问该功能", $this->createMobileUrl('index', array('t'=>1)), "warning");
}
if($member['status']!=1){
	message("您的身份为冻结状态，无法访问该功能", $this->createMobileUrl('index', array('t'=>1)), "warning");
}
		...
		if($succ){
			...
			if($cash_way==1){/*  1.提现到余额 */
				...
				if($cashlog['cash_type']==1){ /* 提现方式为管理员审核 */
					...
					$commission_cash = $font['commission_cash'] ? $font['commission_cash'] : '奖励提现';
					$commission_cash_title = $font['commission_cash_title'] ? $font['commission_cash_title'] : $module_title;
					$desc = $commission_cash_title.$commission_cash;
