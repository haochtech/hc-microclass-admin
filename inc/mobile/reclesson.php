...
if($op=='display'){
	$title = '我的课程邀请';
	$list = pdo_fetchall("SELECT a.*,b.recommend_free_limit,b.recommend_free_num FROM " . tablename($this->table_recommend_activity) . " a LEFT JOIN " .tablename($this->table_lesson_parent). " b ON a.lessonid=b.id WHERE a.uid=:uid ORDER BY a.id DESC  LIMIT " . ($pindex-1) * $psize . ',' . $psize, array(':uid'=>$uid));
	foreach($list as $k=>$v){
		if($v['status']==0 && time() > $v['addtime']+$v['recommend_free_limit']*86400){
			pdo_update($this->table_recommend_activity, array('status'=>-1), array('id'=>$v['id']));
		}
