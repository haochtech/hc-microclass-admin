...
if (time() > $setting['closelast'] + $setting['closespace'] * 60 && $setting['closespace'] != 0) {
	...
	foreach ($nopay_order as $item) {
		$order = pdo_fetch("SELECT id,ordersn,uid,lessonid,coupon,deduct_integral FROM " .tablename($this->table_order). " WHERE id=:id AND status=:status LIMIT 1", array(':id'=>$item['id'],':status'=>0));
		if(empty($order)) continue;

		if($setting['stock_config']==1){
			$site_common->updateLessonStock($order['lessonid'], '1');
		}
