...
if(empty($list)){
	...
	foreach($list as $key=>$rec){
		...
		foreach($list[$key]['lesson'] as $k=>$val){
			...
			$val['tran_time'] = $site_common->tranTime($val['update_time']);
