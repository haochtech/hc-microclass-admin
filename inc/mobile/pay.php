...
if($_GPC['ordertype'] == "buyvip"){
	$order = pdo_get($this->table_member_order, array('id'=>$orderid));
	if ($order['status'] != '0') {
	...
	$order = pdo_get($this->table_order, array('id'=>$orderid));
	if ($order['status'] != '0') {
	...
	$order = pdo_get($this->table_teacher_order, array('id' => $orderid));
	if ($order['status'] != '0') {
