 ...
 */
 ...
 class SiteCommon extends WeModuleSite {
	...
	public function getNavigation($template){
		...
		if(empty($navigation)){
			$navigation = [];
			$nav = pdo_fetchall("SELECT * FROM " .tablename($this->table_navigation)." WHERE uniacid=:uniacid AND is_pc=:is_pc ORDER BY displayorder ASC", array(':uniacid'=>$uniacid, ':is_pc'=>0));
			foreach($nav as $v){
			...
			if(!$navigation['mylesson']){
				$navigation['mylesson'] = array(
					'nav_name'        => '我的订单',
					'unselected_icon' => MODULE_URL."template/mobile/{$template}/images/unselected_mylesson_icon.png",
					'selected_icon'   => MODULE_URL."template/mobile/{$template}/images/selected_mylesson_icon.png",
					'url_link'        => $this->redefineUrl($this->createMobileUrl('mylesson', array('t'=>time()))),
				);
			}
			if(!$navigation['self']){
	 ...
	 */
	public function updateLessonStock($lessonid, $change){
		if($change > 0){
	...
	public function getAgentCommission3($commission_type=0, $lessoncommission, $settingcommission, $price, $uid){
		...
		if ($lessoncommission > 0) {
            ...
            /* 查询用户是否属于其他分销代理级别 */
			$member = pdo_fetch("SELECT b.commission3 FROM " .tablename($this->table_member). " a LEFT JOIN " .tablename($this->table_commission_level). " b ON a.agent_level=b.id WHERE a.uniacid=:uniacid AND a.uid=:uid", array(':uniacid'=>$uniacid,':uid'=>$uid));
	...
	public function getArticleCategory(){
		...
		if(empty($list)){
			$list = pdo_fetchall("SELECT * FROM " .tablename($this->table_article_category). " WHERE uniacid=:uniacid AND status=:status ORDER BY displayorder DESC, id DESC", array(':uniacid'=>$_W['uniacid'], ':status'=>1));
