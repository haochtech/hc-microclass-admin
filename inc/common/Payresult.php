...
class Payresult extends WeModuleSite {
		...
		if($order_type=='L'){
            ...
            if ($params['from'] == 'return') {
				if($lessonorder['lesson_type']==1){
					/* 报名课程 */
					$successUrl = $_W['siteroot'] . "app/index.php?i={$uniacid}&c=entry&status=1&do=mylesson&m=fy_lessonv2";
	...
	/* 获得佣金模通知 */
    public function commissionMessage($data, $acid) {
		$message = array(
			'touser' => $data['openid'],
	...
	/* 发送模版消息 */
    public function send_template_message($messageDatas, $acid = null) {
        global $_W, $_GPC;
        if (empty($acid)) {
	 ...
	 * $setting 全局配置信息
	 */
	public function updateLessonNumber($order, $setting){
